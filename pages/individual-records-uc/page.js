...
(function () {
    ...
    var YEARS = [2015, 2016, 2017, 2018, 2019, 2021, 2022, 2023, 2024];
    var CHAMP_YEARS = { 2015: true, 2017: true, 2019: true, 2022: true, 2024: true };
    var TOP_N = 10;
    ...
    function init() {
        ...
        // Phase 1: fetch 2024 batting + pitching (player names are embedded in each row)
        fetchYear(2024).then(function () {
            // Render 2024 season views immediately
            renderSeasonSections(2024);

            // Phase 2 (background): fetch remaining years in two batches to avoid rate limits
            var remaining = YEARS.filter(function (y) { return y !== 2024; });
            var batchA = remaining.slice(0, 4);   // 2015, 2016, 2017, 2018
            var batchB = remaining.slice(4);       // 2019, 2021, 2022, 2023
